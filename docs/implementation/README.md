...
| `PATCH /auth/me/documento` | Só pra conta sem `documento` ainda (criada via Google) — define CPF/CNPJ (`tipoPessoa` inferido pela quantidade de dígitos) e vira imutável depois. `409 DOCUMENTO_JA_DEFINIDO` se já tem, `409 DOCUMENTO_JA_CADASTRADO` se o CPF/CNPJ já pertence a outra conta |
...
| `DELETE /auth/me` | Deleta a própria conta (exige `senhaAtual`); bloqueada se o usuário for `owner` de evento ou participar do programa de indicação como indicador/indicado — evita eventos órfãos e quebra de histórico financeiro. Mesmo `409` acima pra conta Google — usa o fluxo abaixo |
| `POST /auth/me/exclusao` | Exclusão self-service pra conta sem senha (Google): roda as mesmas checagens de bloqueio de `DELETE /auth/me` e manda um link de confirmação por email (`TokenExclusaoConta`, expira em 24h) em vez de pedir senha |
| `POST /auth/exclusao/confirmar` | Público — confirma o token e executa a exclusão de fato. Base da página `/excluir-conta` (exige clique explícito, ação irreversível) |
...
| `/perfil` | Editar perfil (nome, data de nascimento, **telefone**), trocar email/senha, apagar conta (`/auth/me*`). Conta `usaGoogle: true` esconde os formulários de trocar email/senha (mostram uma nota explicando por quê) e troca o botão de excluir por "Solicitar exclusão por email" (`POST /auth/me/exclusao`) — e mostra o card `CompletarDocumentoCard` (CPF/CNPJ + data de nascimento, sem seletor PF/PJ) enquanto `documento` for nulo, chamando `PATCH /auth/me/documento` |
| `/excluir-conta` | Destino do link de `POST /auth/exclusao/confirmar` — lê `?token=`, exige um clique explícito antes de confirmar (ação irreversível, ao contrário de `/confirmar-email` que confirma sozinha ao montar) |
