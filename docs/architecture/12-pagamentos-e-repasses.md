...
- **Decisão de produto confirmada: a taxa de serviço (os 12% — ver [09-modelo-financeiro.md](09-modelo-financeiro.md)) nunca é reembolsada, mesmo dentro dos 7 dias.** Só o valor do ingresso em si (`valorBruto` menos a taxa) é elegível a reembolso — `valorEstornado` (seção 3) exclui a taxa de serviço. **Isso vale só para esse cancelamento padrão de 7 dias** — o cancelamento via ingresso flexível (seção 43) segue uma regra diferente para a taxa de serviço, não igual a esta.
...
- **Decisão de produto atualizada: só o adicional de 10% de flexibilidade não é reembolsável ao cancelar — a taxa de serviço normal (12%) é devolvida.** O adicional de 10% é o preço da opcionalidade em si (pagou pelo direito de poder cancelar até perto do evento — o direito foi exercido, não faria sentido devolver o que se pagou por ele), então esse fica retido com a plataforma. A taxa de serviço normal, nesse cenário específico, **não** segue a mesma regra do cancelamento padrão (seção 42, onde ela nunca é devolvida) — aqui ela é reembolsada junto com o valor do ingresso. `valorEstornado` (seção 3) = `valorBruto` menos só o adicional de flexibilidade. Fica pendente só como se comporta se o evento for cancelado pelo organizador (cenário já listado como pendente na tabela da seção 39 — esse é um caso diferente de o comprador cancelar por vontade própria).
