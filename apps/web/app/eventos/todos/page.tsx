...
function EventCard({ evento }: { evento: EventoResponse }) {
  ...
  return <Link href={`/e/${evento.id}`} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border/10 bg-card shadow-card transition-all hover:-translate-y-1 hover:border-primary/20 hover:shadow-glow"><div className="relative h-48 shrink-0 bg-cover bg-center" style={{ backgroundImage: `url(${imagemFundo})` }}>{/* evento.temBanner: imagem própria salva como bytes no banco; senão foto de estoque por categoria */}<div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" /><span className="absolute bottom-3 left-3 rounded-lg bg-white/95 px-2.5 py-1 text-[10px] font-bold text-slate-900">{ROTULO_CATEGORIA_EVENTO[evento.categoria]}</span></div><div className="flex min-h-44 flex-1 flex-col p-5"><h3 className="line-clamp-2 min-h-14 text-lg font-semibold tracking-tight text-foreground">{evento.nome}</h3><div className="mt-auto pt-4"><p className="flex items-center gap-2 text-sm text-muted"><CalendarDays size={15} className="shrink-0 text-primary" />{date.toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" })}</p><p className="mt-2 flex items-center gap-2 text-sm text-muted"><MapPin size={15} className="shrink-0 text-primary" /><span className="truncate">{formatarLocalizacaoEvento(evento)}</span></p></div></div></Link>;
}
