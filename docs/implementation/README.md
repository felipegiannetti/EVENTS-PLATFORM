...
| `GET /events/:id/cupons`, `POST .../cupons` | view+ / gestor+ | Lista/cria cupom de desconto do evento (`codigo` único por evento, sempre normalizado pra maiúsculo; `tipo`: `percentual` ≤100 ou `valor_fixo`) |
| `PATCH /events/:id/cupons/:cupomId`, `DELETE .../cupons/:cupomId` | gestor+ | Ativa/desativa (`ativo`) ou remove um cupom. `usos` (contagem de vezes usado) sempre `0` hoje — só passa a incrementar quando existir checkout self-service |
...
| Rota | Papel mínimo | Descrição |
|---|---|---|
| `POST /events/:id/lotes/:loteId/ingressos` | gestor+ | Emissão manual; gera `qrToken` assinado (HMAC-SHA256, `qr-token.util.ts`). `compradorEmail` é **obrigatório** (é o que liga o ingresso ao "Meus ingressos" de um `Usuario`); `compradorNome`/`compradorDocumento`/`linkVendaId` são opcionais. Ao emitir, dispara automaticamente um email de confirmação — **melhor esforço**: se o SMTP não estiver configurado (comum em dev) ou o envio falhar, o erro só vira um `logger.warn`, a emissão **não é desfeita** |
...
| `PATCH /events/:id/ingressos/:ticketId/status` | gestor+ | **"Cancelar"** — marca o ingresso como `cancelado`. Sem confirmação no backend; a tela de Participantes é quem exige confirmação explícita antes de chamar essa rota |
...
| `/meus-ingressos` | Lista os ingressos do usuário logado entre eventos (`GET /tickets/meus`) |
...
| `/eventos/[id]/cupons` | **Cupons de desconto** — cria/ativa/desativa/remove cupons; botão "copiar link" gera a URL pública do evento já com `?cupom=CODIGO` pré-preenchido (`urlPublicaEvento`) |
| `/eventos/[id]/participantes` | **Participantes** — uma linha por ingresso emitido (status, participante, email, tipo, data), com ações **Reenviar** (email), **Editar** (dados do comprador, modal) e **Cancelar** (modal de confirmação antes de chamar a API); botões pra exportar CSV e enviar email em massa aos compradores |
| `/eventos/[id]/checkin` | **Check-in via scanner** — campo de leitura (funciona com leitor USB, que se comporta como teclado) ou câmera do dispositivo (decodificação client-side via `jsQR`, não a `BarcodeDetector` nativa — que não existe no Safari/iOS); histórico das últimas 20 leituras na tela |
