...
| `/perfil` | Editar perfil (nome, data de nascimento, **telefone**), trocar email/senha, apagar conta (`/auth/me*`). Conta `usaGoogle: true` esconde os formulários de trocar email/senha e o botão de excluir (mostram uma nota explicando por quê, em vez de deixar bater no `409 CONTA_SEM_SENHA`) — e mostra o card `CompletarDocumentoCard` (CPF/CNPJ + data de nascimento, sem seletor PF/PJ) enquanto `documento` for nulo, chamando `PATCH /auth/me/documento` |
