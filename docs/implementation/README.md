...
| `POST /tickets/:ticketId/transferir` | qualquer usuário logado | **Self-service** — o próprio comprador (`ingresso.compradorEmail === usuário logado`) transfere o ingresso pra outra conta já cadastrada (`destinatarioEmail`). Exige `evento.transferivel` e, se configurado, respeita `evento.prazoTransferenciaHoras` (rejeita com `409` — `PrazoTransferenciaExpiradoException` — perto demais do evento). Rotaciona o `qrToken` (invalida qualquer captura que o antigo dono tenha guardado) |
| `POST /tickets/:ticketId/cancelar` | qualquer usuário logado | **Self-service** — o próprio comprador cancela o próprio ingresso. Janela padrão de 7 dias corridos da emissão (`PRAZO_CANCELAMENTO_PADRAO_DIAS`, direito de arrependimento — ver [12-pagamentos-e-repasses.md#42](../architecture/12-pagamentos-e-repasses.md#42-cancelamento-de-compra--direito-de-arrependimento-7-dias-e-página-de-políticas)); ingressos com `cancelamentoFlexivel:true` podem cancelar a qualquer momento até o evento começar. Igual ao cancelamento do organizador, não processa reembolso real (não existe checkout) — a taxa de serviço nunca seria reembolsável de qualquer forma, mesmo com checkout (decisão de produto registrada em 12-pagamentos-e-repasses.md) |

### Reservas de ingresso (`POST/GET /events/public/:id/...`) — infraestrutura, sem UI

Endpoints públicos (`ReservasPublicasController`) para segurar uma vaga de lote por até 15 minutos (`PRAZO_RESERVA_MINUTOS`) enquanto um futuro checkout self-service não existe de verdade — ver [11-roadmap.md](../architecture/11-roadmap.md#reserva-de-ingresso-hold-de-15-minutos--infraestrutura-pronta-sem-ui-ainda) para o racional completo. Nenhuma tela consome isso ainda.

| Rota | Descrição |
|---|---|
| `POST /events/public/:id/lotes/:loteId/reservas` | Cria uma reserva (`compradorEmail` opcional). Capacidade reservada de forma atômica, mesmo padrão da emissão — `409 LoteEsgotadoException` se não houver vaga |
