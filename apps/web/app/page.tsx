      ...
      <section id="destaques" className="page-shell scroll-mt-24 !pt-4">
        ...
        {carregandoEventos ? <div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{[1, 2, 3, 4].map((item) => <div key={item} className="h-72 animate-pulse rounded-2xl bg-card" />)}</div> : eventosVisiveis.length > 0 ? (
          <div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{eventosVisiveis.map((evento) => {
            ...
            return <Link key={evento.id} href={`/e/${evento.id}`} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border/10 bg-card shadow-card transition-all hover:-translate-y-1.5 hover:shadow-glow"><div className="relative h-48 shrink-0 overflow-hidden bg-cover bg-center" style={{ backgroundImage: `url(${imagemFundo})` }}><div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" /><span className="absolute bottom-3 left-3 rounded-lg bg-white/95 px-2.5 py-1 text-[10px] font-bold text-slate-900">{ROTULO_CATEGORIA_EVENTO[evento.categoria]}</span></div><div className="flex min-h-40 flex-1 gap-4 p-4"><div className="h-fit min-w-12 rounded-xl bg-primary/10 px-2 py-2 text-center text-primary"><p className="text-sm font-bold leading-4">{String(date.getDate()).padStart(2, "0")}</p><p className="text-[9px] font-bold uppercase">{date.toLocaleDateString("pt-BR", { month: "short" }).replace(".", "")}</p></div><div className="flex min-w-0 flex-1 flex-col"><h3 className="line-clamp-2 min-h-10 font-semibold leading-5 text-foreground">{evento.nome}</h3><p className="mt-2 flex items-center gap-1 text-xs text-muted"><MapPin size={12} className="shrink-0" /><span className="truncate">{formatarLocalizacaoEvento(evento)}</span></p><p className="mt-auto pt-3 text-xs text-muted">{date.toLocaleDateString("pt-BR", { year: "numeric" })}</p></div></div></Link>;
          })}</div>
