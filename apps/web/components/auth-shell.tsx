...
export function AuthShell({ children, title, description }: { children: ReactNode; title: string; description: string }) {
  return (
    <main className="page-shell grid min-h-[calc(100vh-72px)] items-stretch gap-6 lg:grid-cols-[1.05fr_.95fr]">
      ...
      <section className="flex items-center justify-center py-8"><div className="w-full max-w-md"><span className="eyebrow">Bem-vindo à Novyx</span><h1 className="page-title">{title}</h1><p className="page-description">{description}</p><div className="mt-8">{children}</div></div></section>
    </main>
  );
}
